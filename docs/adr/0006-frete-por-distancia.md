...
1. **Cálculo server-side** numa **Vercel Function** `/api/frete` (stateless, atua como
   **proxy**): a chave do provedor vive só no servidor (env var, **não** `REACT_APP_*`, que
   ...
   CEP/endereço.
2. **Provedor:** Mapbox (Geocoding + Matrix), por não exigir cartão de crédito; trocável por
   Google via `/api` se a precisão na periferia decepcionar (o front não muda).
...
- Custo ≈ R$0/mês no volume atual (free tier do Mapbox e do Vercel Hobby).
